...
def load_reaction_subject(feature, subject_id, normalizer,todo):
    '''
    ...
    # if feature == 'Faces1':
    #     feature_path = os.path.join(PATH_TO_RAW_FACE['reaction'], feature)
    #     frames = glob.glob(os.path.join(feature_path, '*.jpg'))
    ...
    if todo == 'regression':
        return features, labels, metas
    if todo == 'multask_class':
        label_multask_path = PATH_TO_LABELS_CLASS['reaction']
        label_multask_df = pd.read_csv(os.path.join(label_multask_path, 'labelOFmulticlassification.csv'))
        label_multask = label_multask_df[label_multask_df.File_ID == subject_id].iloc[:, 1:].values
        return features, labels,label_multask, metas
    if todo == 'multask_mulclass':
        label_multask_mulclass_path = PATH_TO_LABELS_MULCLASS['reaction']
        label_multask_mulclass_df = pd.read_csv(os.path.join(label_multask_mulclass_path, 'MultiLabelclassification.csv'))
        label_multask_mulclass = label_multask_mulclass_df[label_multask_mulclass_df.File_ID == subject_id].iloc[:, 1:].values
        return features, labels, label_multask_mulclass, metas


################# LOAD DATASETS USING THE SPECIFIC METHODS ABOVE #############################################

def load_data(task,todo, paths, feature, emo_dim, normalize=True, win_len=200, hop_len=100, save=False,
              segment_train=True):
    '''
    ...
    data_file_name = f'data_{task}_{todo}_{feature}_{emo_dim+"_" if len(emo_dim)>0 else ""}_{"norm_" if normalize else ""}{win_len}_' \
    ...
    if todo != 'regression':
        data = {'train': {'feature': [], 'label': [], 'meta': [],'label_multask':[]},
                'devel': {'feature': [], 'label': [], 'meta': [],'label_multask':[]},
                'test': {'feature': [], 'label': [], 'meta': [],'label_multask':[]}}
    else:
    ...
    for partition, subject_ids in partition2subject.items():
        ...
        for subject_id in tqdm(subject_ids):
            if task == 'stress':
                ...
                if todo != 'regression':
                    features, labels,labels_task, metas = load_reaction_subject(feature=feature,todo=todo ,subject_id=subject_id,
                                                                    normalizer=normalizer)
                else:
                    features, labels, metas = load_reaction_subject(feature=feature, todo=todo,subject_id=subject_id,
                                                                normalizer=normalizer)
            ...
            if todo != 'regression':
                data[partition]['label_multask'].extend(labels_task)
