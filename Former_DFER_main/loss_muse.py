...
class WrappedPRSLoss(nn.Module):
    ...
    def forward(self,preds,labels, seq_lens=None):
        pearson = self.cos(preds - preds.mean(dim=1, keepdim=True), labels - labels.mean(dim=1, keepdim=True))
        # pearson = self.cos(preds,labels)
        pearson = pearson.mean()
        return 0.2*(1-pearson)+0.8*self.loss_fn(preds,labels)
    ...
    #     return r[0]
    #
    # def mean_pearsons(self,preds, labels):
