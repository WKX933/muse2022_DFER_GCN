...
class ResNet(nn.Module):   # S-Former after stage3
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)     # torch.Size([1, 256, 7, 7])  [1024,256,7,7]
        b_l, c, h, w = x.shape
        x = x.reshape((b_l, c, h*w))
        x = x.permute(0, 2, 1) #[1024,49,256]
        b, n, _ = x.shape
        x = x + self.pos_embedding[:, :n]
        x = self.spatial_transformer(x) #[1024,49,256]
        x = x.permute(0, 2, 1)
        x = x.reshape((b, c, h, w))
        x = self.layer4(x)     # torch.Size([1, 512, 4, 4])
        x = self.avgpool(x)#[1024,512,1,1]
        x = torch.flatten(x, 1) #[1024,512]
