...
def extract(data_loader, model, layer_name):
    ...
    with torch.no_grad():
        features, timestamps = [], []
        for imgs, ids in tqdm(data_loader):
            imgs = imgs.cuda()
...
def main(params):
    ...
    # out: feature csv dir
    save_dir = os.path.join(config.PATH_TO_FEATURES[params.task], f"{params.model_name.split('_')[0]}face_{params.layer_name.split('_')[0]}_1")
    ...
    pretrained_dir = os.path.join(config.PATH_TO_PRETRAINED_MODELS[params.task], 'ferplus') # directory of pre-trained models
