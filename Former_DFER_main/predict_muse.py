...
from models.GCN_MLGCN import Model
...
import numpy as np
# import datetime
from dataloader.dataset_muse_predict import train_data_loader, test_data_loader,pred_data_loader
...
parser.add_argument('--eval_model', type=str,
                    default='/data6/wangkexin/muse2022/Former_DFER_main/checkpoint/[06-13]-[22:13]-set1-model_best.pth',
...
def main():
    ...
    # pred_data = pred_data_loader(data_set=args.data_set)
    ...
    val_loader = torch.utils.data.DataLoader(train_data,
                                             ...
                                             num_workers=args.workers,
                                             pin_memory=True)
    # test_loader = torch.utils.data.DataLoader(pred_data,
    #                                          batch_size=args.batch_size,
    #                                          shuffle=False,
    #                                          num_workers=args.workers,
    #                                          pin_memory=True)
    ...
    best_val_score = -1
    best_model_file = ''
    # val_score,_ = validate(val_loader, model, criterion, args,predict=False)
    ...
    if args.predict:  # Make predictions for the test partition; this option is set if there are no test labels
        ...
        validate(val_loader, model, criterion, args,predict=True)
    print('Done.')
...
def validate(val_loader, model, criterion, args,predict):
    losses = AverageMeter('Loss', ':.4f')
    # top1 = AverageMeter('Accuracy', ':6.3f')
    ...
    full_metas = []
    with torch.no_grad():
        for i, (images, target,metas) in enumerate(val_loader):
            images = images.cuda()
            target = target.cuda()
            ...
            full_metas.append(metas[:args.batch_size])
            # if i % args.print_freq == 0:
            #     progress.display(i)
        ...
        if predict:
            log_file_name = '{}_[{}]_[{}_{}]'.format(datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"),
                                                       'DFER',
                                                       args.lr, args.batch_size)
            prediction_path = os.path.join('/data6/wangkexin/muse2022/results/prediction_muse/reaction',log_file_name)
            write_predictions('reaction', full_metas, full_preds, prediction_path, filename='predictions.csv')
            return
...
def write_reaction_predictions(full_metas, full_preds, csv_dir, filename):
    x = [int(i) for item in full_metas for i in item]
    meta_arr = np.row_stack(x)
    preds_arr = np.row_stack(full_preds)
    pred_df = pd.DataFrame(columns=['File_ID']+REACTION_LABELS)
    pred_df['File_ID'] = meta_arr.tolist()
